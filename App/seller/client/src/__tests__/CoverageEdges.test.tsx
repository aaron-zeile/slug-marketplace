...
vi.mock('@mui/x-charts/LineChart', () => ({
  LineChart: ({series}: {series: Array<{label: string; valueFormatter?: (value: number | null) => string}>}) => (
    <div>
      {series.map((entry) => (
...
describe('coverage edges', () => {
  ...
  it('redirects unauthenticated sellers to shopper', async () => {
    ...
    await waitFor(() => expect(replace).toHaveBeenCalledWith('http://localhost:3000/'))
  })
  ...
  it('renders plural review copy for multiple reviews', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async () => ({
        ok: true,
        json: async () => ({reviews: [review, {...review, id: 'review-2'}]}),
      })),
    )
  ...
  it('shows an edit dialog price validation message', async () => {
    ...
    expect({
      priceErrorVisible: screen.queryByText('Price must be at least $0.01.') !== null,
  ...
  it('renders the listing load error label without an error provider', async () => {
    ...
    expect(await screen.findByText('Listings failed to load')).toBeInTheDocument()
  })
  ...
  it('removes a listing after deleting it from the listings table', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', {name: 'Delete USB Hub'}))
  ...
  it('marks a shipping order delivered', async () => {
    ...
    await waitFor(() => expect(fetchMock.mock.calls[1]?.[1]).toEqual(
  ...
  it('marks an ordered order shipped', async () => {
    ...
    renderWithProviders(<Sales />)
    const shipButtons = await screen.findAllByRole('button', {name: 'Mark shipped'})
  ...
  it('renders the sales load error label without an error provider', async () => {
    ...
    expect(await screen.findByText('Orders failed to load')).toBeInTheDocument()
  })
  ...
  it('falls back when the star distribution request fails without an error provider', async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledWith('/seller/api/analytics/star-distribution'))
  })
})
